...
//
// CORREÇÕES v2:
//   - URL_API removida daqui (definida apenas em tema.js)
//   - toggleDarkMode removida daqui (definida em tema.js)
//   - Sem redirecionamento para arquivos de tema inexistentes
//   - Aviso de sessão expirada via parâmetro de URL
// ============================================================

// URL_API e toggleDarkMode vêm de tema.js — NÃO redefinir aqui
...
async function init() {
  ...
  try {
    ...
    if (!data) {
      // Bio falhou — pode ser problema de rede ou CORS
      // Não bloqueia: mantém tema do cache e exibe o que tiver
      hideLoading();
      return;
    }
...
async function confirmCode() {
  ...
  try {
    ...
    if (data.status === 'ok') {
      closeOtpModal();
      // Passa flag de primeiro acesso para linha-tempo.html
...
document.addEventListener('DOMContentLoaded', () => {
  // Ícone do tema (tema.js já aplicou o tema, só precisa atualizar o ícone)
  const isDark = document.documentElement.classList.contains('dark');
  const icon   = document.getElementById('themeIcon');
  if (icon) icon.className = isDark ? 'fas fa-sun' : 'fas fa-moon';

  // Listener blur no campo email
